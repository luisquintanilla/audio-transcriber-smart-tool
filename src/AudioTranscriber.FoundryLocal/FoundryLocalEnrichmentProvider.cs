    ...
    : ITranscriptChapterEnricher, ITranscriptOverallSummaryAssembler, IAsyncDisposable
{
    ...
    private async Task<FoundryLocalOperationLease> AcquireOperationAsync(
        ...
        bool retryInitializationFailure = true)
    {
        ...
        lock (lifecycleGate)
        {
            ...
            initialization = sessionInitialization ??= StartInitialization();
        }
        ...
        {
            ReleaseOperation();
            if (retryInitializationFailure &&
                !cancellationToken.IsCancellationRequested &&
                (initialization.IsCanceled ||
                     ...
                     is OperationCanceledException))
            {
                ...
                return await AcquireOperationAsync(
                        cancellationToken,
                        retryInitializationFailure: false)
                    .ConfigureAwait(false);
            }
            ...
            {
                if (ReferenceEquals(sessionInitialization, initialization) &&
                    (initialization.IsFaulted || initialization.IsCanceled))
                {
                    sessionInitialization = null;
                }
            }
    ...
    private Task<FoundryLocalRuntimeSession> StartInitialization()
    {
        ...
        CancellationTokenSource cancellation)
    {
        ...
        catch (Exception exception)
        {
            lock (lifecycleGate)
            {
                if (ReferenceEquals(sessionInitialization, completion.Task))
                {
                    sessionInitialization = null;
                }
            }
