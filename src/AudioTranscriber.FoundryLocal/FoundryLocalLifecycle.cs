...
internal sealed class FoundryLocalSdkModelLifecycle : IFoundryLocalModelLifecycle
{
    ...
    public Task<bool> IsCachedAsync(CancellationToken cancellationToken) =>
        model.IsCachedAsync(cancellationToken);

    public Task DownloadAsync(CancellationToken cancellationToken) =>
        model.DownloadAsync(null, cancellationToken);
...
internal sealed class FoundryLocalModelLeaseRegistry
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var entry = GetOrCreateEntry(model.ModelId);
        await entry.Gate.WaitAsync(cancellationToken).ConfigureAwait(false);
        try
        {
            if (!await model.IsCachedAsync(cancellationToken).ConfigureAwait(false))
            {
                await model.DownloadAsync(cancellationToken).ConfigureAwait(false);
            }
        }
        finally
        {
    ...
    private Entry GetOrCreateEntry(string modelId)
    {
        lock (entriesGate)
        {
