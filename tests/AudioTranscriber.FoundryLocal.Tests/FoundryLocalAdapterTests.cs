...
public sealed class FoundryLocalAdapterTests
{
    ...
    [Fact]
    public void Chapter_prompt_uses_json_null_for_missing_source_id()
    {
        var artifact = CreateArtifact(
    ...
    [Fact]
    public async Task Queued_acquire_keeps_entry_reference_counted_during_final_release()
    {
        var registry = new FoundryLocalModelLeaseRegistry();
        var model = new FakeModelLifecycle("model-id", isLoaded: false);
        var first = await registry.AcquireAsync(model, CancellationToken.None);
        model.BlockNextUnload();

        var firstRelease = first.DisposeAsync().AsTask();
        await model.UnloadStarted.Task;
        var second = registry.AcquireAsync(model, CancellationToken.None);
        var third = registry.AcquireAsync(model, CancellationToken.None);

        model.ReleaseUnload.SetResult();
        var secondLease = await second;
        var thirdLease = await third;

        Assert.Equal(2, model.LoadCount);
        Assert.Equal(1, model.UnloadCount);
        ...
        await firstRelease;
        Assert.Equal(2, model.UnloadCount);
    }

    [Fact]
    public async Task Concurrent_cache_preparation_downloads_once_per_model()
    {
        var registry = new FoundryLocalModelLeaseRegistry();
        var model = new FakeModelLifecycle("model-id", isLoaded: false);
        model.BlockNextDownload();

        var first = registry.EnsureCachedAsync(
            model,
            allowDownload: true,
            CancellationToken.None);
        await model.DownloadStarted.Task;
        var second = registry.EnsureCachedAsync(
            model,
            allowDownload: true,
            CancellationToken.None);

        model.ReleaseDownload.SetResult();
        await Task.WhenAll(first, second);

        Assert.Equal(1, model.DownloadCount);
    }

    [Fact]
    public async Task Queued_acquire_reuses_entry_after_load_failure()
    {
        var registry = new FoundryLocalModelLeaseRegistry();
        var model = new FakeModelLifecycle("model-id", isLoaded: false);
        model.FailNextLoadAfterBlocking();

        var first = registry.AcquireAsync(model, CancellationToken.None);
        await model.LoadStarted.Task;
        var second = registry.AcquireAsync(model, CancellationToken.None);

        model.ReleaseLoad.SetResult();
    ...
    [Fact]
    public void Foundry_chat_service_is_only_available_for_unkeyed_requests()
    {
        Assert.True(
            FoundryLocalSdkChatClient.CanProvideUnkeyedService(
                typeof(IChatClient),
                serviceKey: null));
        Assert.False(
            FoundryLocalSdkChatClient.CanProvideUnkeyedService(
    ...
    [Fact]
    public void Foundry_chat_request_options_use_typed_sampling_settings()
    {
        var nativeOptions = FoundryLocalSdkChatClient.CreateRequestOptions(
            new ChatOptions
            {
                Temperature = 0.25f,
                MaxOutputTokens = 321
        ...
        Assert.Equal(0.25f, nativeOptions.Search.Temperature);
        Assert.Equal(321, nativeOptions.Search.MaxOutputTokens);
    }
    ...
    private sealed class FakeModelLifecycle : IFoundryLocalModelLifecycle
    {
        ...
        private bool blockNextUnload;
        private bool blockNextLoad;
        private bool failNextLoad;
        private bool blockNextDownload;
        ...
        public TaskCompletionSource UnloadStarted { get; } =
            new(TaskCreationOptions.RunContinuationsAsynchronously);

        public TaskCompletionSource ReleaseUnload { get; } =
            new(TaskCreationOptions.RunContinuationsAsynchronously);
        ...
        public void BlockNextUnload() => blockNextUnload = true;

        public void FailNextLoadAfterBlocking()
        {
            blockNextLoad = true;
            failNextLoad = true;
        }

        public void BlockNextDownload() => blockNextDownload = true;

        public Task<bool> IsCachedAsync(CancellationToken cancellationToken)
        {
            cancellationToken.ThrowIfCancellationRequested();
            return Task.FromResult(DownloadCount > 0);
        }
        ...
        public async Task DownloadAsync(CancellationToken cancellationToken)
        {
            ...
            if (blockNextDownload)
            {
                ...
                await ReleaseDownload.Task.ConfigureAwait(false);
            }
        }
        ...
        public async Task UnloadAsync(CancellationToken cancellationToken)
        {
            ...
            if (blockNextUnload)
            {
                ...
                await ReleaseUnload.Task.ConfigureAwait(false);
            }
